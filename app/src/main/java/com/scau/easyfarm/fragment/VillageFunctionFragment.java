...
public class VillageFunctionFragment extends BaseFunctionFragment{
    ...
    public void initModuleList() {
        ...
        if (Module.existModule(u.getModuleList(), Module.MODULE_SERVICE_APPLY)){
            Module serviceApply = new Module(SimpleBackPage.VILLAGE_SERVICE_APPLY,"服务申请",R.drawable.func_service_apply_icon,null);
            moduleList.add(serviceApply);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_SERVICE_PROOF)){
            Module serviceProof = new Module(SimpleBackPage.VILLAGE_SERVICE_PROOF,"服务佐证",R.drawable.func_service_proof_icon,null);
            moduleList.add(serviceProof);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_SERVICE_VERIFY)){
            Module serviceVerify = new Module(SimpleBackPage.VILLAGE_SERVICE_VERIFY_VIEWPAGER,"服务审批",R.drawable.func_service_apply_icon,null);
            moduleList.add(serviceVerify);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_SERVICE_STATISTICS_EXPERT)){
            Module serviceStatisticsExpert = new Module(SimpleBackPage.MYSERVICE_MONTH_STATISTICS,"服务统计",R.drawable.func_service_statistic_icon,null);
            moduleList.add(serviceStatisticsExpert);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_SERVICE_STATISTICS_VERIFIER)){
            Module serviceStatisticsVerifier = new Module(SimpleBackPage.SERVICE_MONTH_STATISTICS,"服务统计",R.drawable.func_service_statistic_icon,null);
