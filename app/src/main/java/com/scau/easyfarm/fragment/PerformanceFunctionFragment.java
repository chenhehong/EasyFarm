...
public class PerformanceFunctionFragment extends BaseFunctionFragment{
    ...
    public void initModuleList() {
        ...
        if (Module.existModule(u.getModuleList(), Module.MODULE_PERFORMANCE_APPLY)){
            Module performanceApply = new Module(SimpleBackPage.PERFORMANCE_APPLY_LIST,"绩效申请",R.drawable.func_agriculture_knowledge_base_icon,null);
            moduleList.add(performanceApply);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_PERFORMANCE_STATISTICS_EXPERT)){
            Module performanceStatisticsExpert = new Module(SimpleBackPage.MYPERFORMANCE_MONTH_STATISTICS,"绩效统计",R.drawable.func_village_service_performance_icon,null);
        ...
        if (Module.existModule(u.getModuleList(), Module.MODULE_PERFORMANCE_STATISTICS_VERIFIER)){
            Module performanceStatisticsVerifier = new Module(SimpleBackPage.PERFORMANCE_MONTH_STATISTICS,"绩效统计",R.drawable.func_village_service_performance_icon,null);
            moduleList.add(performanceStatisticsVerifier);
        }
        if (Module.existModule(u.getModuleList(), Module.MODULE_PERFORMANCE_VIREFY)){
            Module performanceVerify = new Module(SimpleBackPage.PERFORMANCE_VERIFY_VIEWPAGER,"绩效审核",R.drawable.func_agriculture_knowledge_base_icon,null);
